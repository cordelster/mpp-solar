""" mppsolar / protocols / pi30revo_v2.py """
import logging
...
log = logging.getLogger("pi30revo_v2")
...
class pi30revo_v2(AbstractProtocol):
    "PI30_v2 protocol handler for REVO and similar inverters"
    def __str__(self):
        return "PI30_v2 protocol handler for REVO and similar inverters"

    def __init__(self, *args, **kwargs) -> None:
        super().__init__()
        self._protocol_id = b"PI30REVO_v2"
    ...
    def get_full_command(self, command) -> bytes:
        """
        Override the default get_full_command as its different for PI30REVO_v2
        """
        ...
        if command.startswith('PSET'):
            ...
            log.debug("Using PI30_v2 CRC checksum approach for command: %s", self._command)
            # calculate the CRC
